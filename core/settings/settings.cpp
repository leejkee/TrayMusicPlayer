...
public:
    ...
    static inline const auto KEY_MUSIC_DIRECTORY =
            QStringLiteral("MusicDirectory");
    static inline const auto KEY_USER_LISTS = QStringLiteral("UserLists");
    static inline const auto KEY_DEFAULT_VOLUME =
    ...
    static inline const auto SETTINGS_TEMPLATE_FILE =
            QStringLiteral("win32/init_win32.json");
...
SettingsPrivate::SettingsPrivate(const QString& objName)
    : Log(Log::QLogger(objName)),
      m_settingsPath(
...
void SettingsPrivate::parseJson()
{
    if (newSettingsFile(m_settingsPath))
    {
        ...
        m_localMusicList = jsonObj[KEY_MUSIC_DIRECTORY].toVariant().
                toStringList();
...
bool SettingsPrivate::newSettingsFile(const QString& file) const
{
    ...
    {
        if (const QString source = PROJECT_PATH + SETTINGS_DIRECTORY +
                SETTINGS_TEMPLATE_FILE; !QFile::copy(
                                                     source
                                                     , file))
        {
            Log.log(Log::QLogger::LogLevel::Error
        ...
        else
        {
            Log.log(Log::QLogger::LogLevel::Info
                    , "Successfully copy from template [" + source +
                    "] as settings file: " + file);
        }
    }
    else
    {
        Log.log(Log::QLogger::LogLevel::Info
                , "Settings file already exists, read it: " + file);
    }
    return true;
}
...
void Settings::addLocalMusicDirectory(const QString& path)
{
    if (!QDir(path).exists())
    {
        d->Log.log(Log::QLogger::LogLevel::Error, "Attempted to add an empty or non-existent local directory: " + path);
        return;
    }
