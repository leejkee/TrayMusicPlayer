...
namespace Tray::Core {
    ...
    public:
        ...
        void updatePlaylistIfPlayingThis(const QString &key, const QList<Song> &list);

        void handleSwitchPlaylistAndPlayIndex(const QString &key, const QList<Song> &list, int index);
    ...
    void CorePrivate::initListCache() {
        QMetaObject::invokeMethod(m_listCache, "init", Qt::QueuedConnection,
                                  Q_ARG(QStringList, m_settings->getLocalMusicDirectories()),
                                  Q_ARG(QStringList, m_settings->getKeysUserPlaylist()));
    }

    void CorePrivate::updatePlaylistIfPlayingThis(const QString &key, const QList<Song> &list) {
        if (m_playList->getListKey() == key) {
            m_playList->loadMusicList(key, list);
        }
    }

    void CorePrivate::handleSwitchPlaylistAndPlayIndex(const QString &key, const QList<Song> &list, const int index) {
        m_playList->loadMusicList(key, list);
    ...
    void Core::createConnections() {
        ...
        connect(d->m_listCache, &ListCache::signalInitCompleted,
                this, [this]() {
                    Q_EMIT signalRequestTitleList(LOCAL_LIST_KEY);
                    d->Log.log(Log::QLogger::LogLevel::Info, "List cache initialization completed");
                });

        connect(d->m_player, &Player::signalPlayingChanged,
                this, [this](const bool b) { Q_EMIT signalNotifyUiPlayingStatusChanged(b); });

        connect(d->m_playList, &PlayList::signalCurrentMusicChanged,
                this, [this](const qsizetype index, const QString &name, const int duration) {
                    Q_EMIT signalNotifyUiCurrentMusicChanged(static_cast<int>(index), name, duration);
                });

        connect(d->m_player, &Player::signalIsMuted,
                this, [this](const bool b) { Q_EMIT signalIsMuted(b); });

        connect(d->m_player, &Player::signalPositionChanged,
        ...
        connect(d->m_listCache, &ListCache::signalUserPlaylistCreated,
                this, [this](const QString &key) { Q_EMIT signalNotifyUiToAddUserPlaylist(key); });
        connect(d->m_listCache, &ListCache::signalUserPlaylistDeleted,
                this, [this](const QString &key) { Q_EMIT signalNotifyUiToRemoveUserPlaylist(key); });
        // settings --> ui (viewWidget)
        connect(d->m_settings, &Settings::signalUserKeySetsChanged,
                this, [this](const QStringList &list) { Q_EMIT signalNotifyUiUserKeySetsChanged(list); });
        /// User playlist Add/Remove

        /// List cache changed
        connect(d->m_listCache, &ListCache::signalNotifyPlayListCacheModified,
                this, [this](const QString &key, const QList<Song> &list) {
                    d->updatePlaylistIfPlayingThis(key, list);
                });
        connect(d->m_listCache, &ListCache::signalNotifyUiCacheModified,
                this, [this](const QString &key, const QStringList &titleList) {
        ...
        /// Switch view list
        connect(this, &Core::signalRequestTitleList, d->m_listCache, &ListCache::respondMusicTitleList);
        ...
        connect(d->m_listCache, &ListCache::signalRespondPlayListSwitchAndPlayIndex,
                this, [this](const QString &key, const QList<Song> &list, const int index) {
    ...
    void Core::addMusicToList(const QString &sourceListKey, const QString &destinationListKey, const int index) {
        Q_EMIT signalCopyMusicFromListToList(sourceListKey, destinationListKey, index);
    }
