...
CorePrivate::CorePrivate(QObject* p_core)
    : p_ptr(p_core)
{
    ...
    LOG_INFO("Initializing Core successfully");
}
...
void CorePrivate::initListCache()
{
    QMetaObject::invokeMethod(
                              ...
                              , Qt::QueuedConnection
                              , Q_ARG(QStringList
                                      , m_settings->getLocalMusicDirectories())
                              , Q_ARG(QStringList
                                      , m_settings->getKeysUserPlaylist()));
}
...
void CorePrivate::handleSwitchPlaylistAndPlayIndex(
    const QString& key
    , const QList<MusicMetaData>& list
    , const int index)
{
...
{
    connect(d->m_listCacheThread
            , &QThread::started
            , this
            , [this]() { d->initListCache(); });

    connect(d->m_listCache
            , &ListCache::signalInitCompleted
            , this
            , [this]()
            {
                Q_EMIT signalRequestTitleList(LOCAL_LIST_KEY);
                LOG_INFO("List cache initialization completed");
            });
    ...
    connect(
            ...
            , [this](const int mode)
            {
                Q_EMIT signalNotifyUiPlayModeChanged(mode);
            });
    ...
    connect(d->m_listCache
            ...
            , this
            , [this](const QString& key, const QList<MusicMetaData>& list)
            {
    ...
    connect(this
            , &Core::signalRemoveUserPlaylistFromCache
            , d->m_listCache
            , &ListCache::deleteUserPlaylist);
    connect(this
            , &Core::signalAddUserPlaylistToCache
            , d->m_listCache
    ...
    /// Switch view list
    connect(this
            , &Core::signalRequestTitleList
            , d->m_listCache
            , &ListCache::respondMusicTitleList);
    connect(d->m_listCache
            , &ListCache::signalSendUiCurrentTitleList
            , this
    ...
    connect(this
            , &Core::signalCopyMusicFromListToList
            , d->m_listCache
            , &ListCache::copyMusicFromListToList);

    connect(this
            , &Core::signalRemoveMusicFromList
            , d->m_listCache
            , &ListCache::removeSongFromListByTitle);

    connect(this
            , &Core::signalRequestSwitchPlaylistAndPlayIndex
            , d->m_listCache
            , &ListCache::handleSwitchPlaylistAndPlayIndex);

    connect(
            d->m_listCache
            , &ListCache::signalRespondPlayListSwitchAndPlayIndex
            , this
            , [this](const QString& key
                     , const QList<MusicMetaData>& list
                     , const int index)
