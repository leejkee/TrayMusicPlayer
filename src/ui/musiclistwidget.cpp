...
namespace Tray::Ui {
    MusicListWidget::MusicListWidget(QWidget *parent)
        : QWidget(parent) {
        m_localListButton = new Panel::StyleButton({LOCAL_LIST_KEY, Res::MusicListSVG, Res::BUTTON_NORMAL_QSS, 30, 0 }, this);
        m_expandButton = new Panel::StyleButton({EXPAND_BTN_TEXT, Res::UpSVG, Res::BUTTON_EXPAND_QSS, 30, 60}, this);
        m_addButton = new Panel::StyleButton({{}, Res::AddSVG, Res::BUTTON_ADD_QSS, 0, 0}, this);
    ...
    void MusicListWidget::createConnections() {
        connect(m_localListButton, &Panel::StyleButton::signalButtonClicked, this,
    ...
    void MusicListWidget::appendUserButton(const QString &playlistName) {
        auto *button = new Panel::StyleButton({playlistName, Res::MusicListSVG, Res::BUTTON_NORMAL_QSS, 30, 0}, this);
        m_UserButtonHash[playlistName] = button;
        m_buttonLayout->addWidget(button);
        button->setContextMenuPolicy(Qt::CustomContextMenu);
        connect(button, &Panel::StyleButton::signalButtonClicked, this, &MusicListWidget::handleMusicButtonClicked);
        connect(button, &Panel::StyleButton::customContextMenuRequested, this, &MusicListWidget::handleContextMenu);
    }
